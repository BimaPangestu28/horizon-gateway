...
func main() {
	...
	if err != nil {
		logger.Fatal("Failed to create router", "error", err)
	}
	...
	// Create server
	srv, err := server.NewServer(cfg, configWatcher, logger, proxyHandler, metricsHandler)
	if err != nil {
		logger.Fatal("Failed to create server", "error", err)
	}

	// Create admin server - fixed function call to match signature
	adminSrv, err := server.NewAdminServer(cfg, configWatcher, logger)
	if err != nil {
		logger.Fatal("Failed to create admin server", "error", err)
	}

	// Start the config watcher
	if err := configWatcher.Start(); err != nil {
		logger.Fatal("Failed to start config watcher", "error", err)
	}
	...
	configWatcher.RegisterCallback(func(newCfg *config.Config) error {
		...
		if err := srv.UpdateConfig(newCfg); err != nil {
			logger.Error("Failed to update server config", "error", err)
			return err
		}

		if err := adminSrv.UpdateConfig(newCfg); err != nil {
			logger.Error("Failed to update admin server config", "error", err)
			return err
		}
	...
	go func() {
		...
		if err := adminSrv.Start(); err != nil {
			logger.Fatal("Admin server failed to start", "error", err)
		}
	}()
